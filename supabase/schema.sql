...
begin
  ...
  for v_index in 1..8 loop
    ...
    select auth.uid(), p_player_ids[v_index], 'Season 05', v_period, category, p_slots[v_index], p_player_ids[v_index] = p_captain_player_id
...
begin
  if auth.uid() is null then raise exception 'Authentication required'; end if;
  v_period := coalesce(p_period, (select gameweek from public.fantasy_settings where id = 'current'), 1);
  if not public.fantasy_period_is_open(v_period) then raise exception 'Captain changes open only after the active deadline ends'; end if;
...
begin
  if auth.uid() is null then raise exception 'Authentication required'; end if;
  v_period := coalesce(p_period, (select gameweek from public.fantasy_settings where id = 'current'), 1);
  if not public.fantasy_period_is_open(v_period) then raise exception 'Power-ups open only after the active deadline ends'; end if;
