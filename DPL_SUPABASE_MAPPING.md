...
When the point-tracking app begins storing results, add a read-only view or table with at least `player_id`, `gameweek_id`, `game_points`, `wins`, `losses`, `overall_points`, and `updated_at`. The fantasy app can then consume that view without duplicating score-entry workflows. Do not use the Supabase service-role key in browser code.
